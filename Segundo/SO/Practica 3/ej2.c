...
void main(){
    ...
    for (i=0; i<100; i++){ //Le doy 100 unidades de procesado
        ...
        //Calculamos el proceso que entra cuando no hay ninguno en el procesador (procesando = 8)
        if (procesando == 8){
            min = 10000; //reseteamos el minimo para evitar problemas con procesos terminados
            for(j=0; j<NPROC; j++){
                if (listos[j] != 8){ //8 es nuestro numero de control
                    if(procesos[listos[j]].t_ejec < min){
                        min = procesos[listos[j]].t_ejec;
                        procesando = listos[j];
                    }
                }
            }
